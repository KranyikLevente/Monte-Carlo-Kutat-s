...
class Grid:
    def __init__(self, num_voxels, voxel_size):
        ...
        # PE: kisebb abszorpcioval lehet jobban nezne ki a fluxus plot
    ...
    def which_voxel(self, position):
        ...
        # PE: ha clippelni kell, az baj, dobjunk errort inkabb?
...
def woodcock_tracking(particle, grid, global_real_paths):
    # PE: ez a valtozo kell valamihez?
    ...
    while particle.state == 'active':
        ...
        if np.any(particle.position < 0) or np.any(particle.position >= grid.total_size):
            ...
            break
        # PE: flux tally az itt a track length lenne? ott fontos, hogy a voxel
        #     hatarain belul kell szamolni a tavokat.
        #     collision tally eseten 1/Sigma_maj a kontribucio
        grid.flux_tally[current_voxel] += random_distance
        # PE: itt miert relevans a target voxel? A hkm-eket az utkozes helyen
        #     nezzuk, nem?
...
def surface_tracking(particle, grid, global_real_paths):
    ...
    while dist_to_collide > 0 and particle.state == 'active':
        # PE: tracking nem valos tavolsag alapjan, optikai uthosszat tudjuk mintavételezni
        current_voxel = grid.which_voxel(particle.position)
...
def combined_tracking(particle, grid, global_real_paths):
    distance_traveled = 0.0
    # PE: hasznaljuk ezt a valtozot?
    # PE: surface trackinghez hasonloan addig kene menni mig "van ut"
    while particle.state == 'active':
        current_voxel = grid.which_voxel(particle.position)
        
        d_bound = get_distance_to_boundary(particle.position, particle.direction, grid.voxel_size)
        d_woodcock = np.random.exponential(scale=1.0 / grid.sigma_maj)
        # PE: woodcock csak az utolso voxelben szukseges
...
compare_methods(N_particles=4000)